function rowIdx = dg_reorderRowsGenerate(dg_cfg)
...
% type and objective. 
%
% the rowIdx output is then verified, and its verification checks can be seen when dg_cfg.verbose = true
%
% INPUT:
%   dg_cfg              - struct with fields:
...
%
% Notes:
% PERMUTATION is done differently depending if there are repeated measures:
...
%
% BOOTSTRAP is always done by resamples with replacement full observation
...
if ~isfield(dg_cfg.analysis,'designCode') || ~isnumeric(dg_cfg.analysis.designCode) || numel(dg_cfg.analysis.designCode)~=2
    error('dg_cfg.analysis.designCode must be a 1x2 numeric vector as generated by dg_parseDesign');
end

%% shortcuts

dataStruct = dg_cfg.analysis.dataStruct;
nIterations = double(dg_cfg.analysis.nIterations);
...
[unique_obsID,~,obsIdxPerRow] = unique(dg_cfg.analysis.dataStruct.observationID,'stable');
nUnique_obsID = numel(unique_obsID);

% independent group factor columns (if any)
indF_col_idx = startsWith(dg_cfg.analysis.dataStruct.Properties.VariableNames, 'indFactor');
nUnique_indFactors = nnz(indF_col_idx);

if nUnique_indFactors > 0
    % create group identifier from indFactor columns
    indFactorCols = dg_cfg.analysis.dataStruct(:, indF_col_idx);
...
rmF_col_idx = startsWith(dg_cfg.analysis.dataStruct.Properties.VariableNames, 'repFactor');
nUnique_repFactors = nnz(rmF_col_idx);
nUnique_RMLevelsPerFactors = nan(1, nUnique_repFactors);
colCounter = 0;
for rmIdx = find(rmF_col_idx)
    colCounter = colCounter + 1;
    nUnique_RMLevelsPerFactors(1, colCounter) = numel(unique(dg_cfg.analysis.dataStruct{:, rmIdx}));
end
...
key = [char(dg_cfg.analysis.objective) ' + ' num2str(dg_cfg.analysis.designCode)];
...
if strcmp(dg_cfg.analysis.objective, 'permutationH0testing')
    if strcmp(dg_cfg.analysis.designCode, '1  0') % designCode [1 0]
...
if strcmp(dg_cfg.analysis.objective, 'bootstrapStability')
    for itIdx = 2:nIterations % start from 2 since itIdx 1 is original data
